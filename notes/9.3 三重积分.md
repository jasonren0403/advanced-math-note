...
* $\displaystyle \iint_D f(x,y)\mathrm dx \mathrm dy$：曲顶柱体面积，薄片质量
...
例如：$\Omega: x^2+y^2+z^2\le R^2$，有 $\displaystyle \iiint_\Omega x^2 \mathrm dV=\iiint_\Omega y^2=\iiint_\Omega z^2=\dfrac{1}{3}\iiint_\Omega (x^2+y^2+z^2) \mathrm dV$
## 三、三重积分的计算方法
### 直角坐标系
1. 投影法（“先一后二”）
    ...
    * 投影到 $yOz$ 面上：$\displaystyle I=\iint_{D_{yz}} \mathrm dy \mathrm dz \int_{\phi_1(y,z)}^{\phi_2(y,z)} f(x,y,z) \mathrm dz$（$D_{yz}$ 为 $\Omega$ 在 $yOz$ 上投影区域）
    * 投影到 $xOz$ 面上：$\displaystyle I=\iint_{D_{xz}} \mathrm dx \mathrm dz \int_{\phi_1(x,z)}^{\phi_2(x,z)} f(x,y,z) \mathrm dz$（$D_{xz}$ 为 $\Omega$ 在 $xOz$ 上投影区域）
